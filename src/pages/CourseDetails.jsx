...
const CourseDetails = () => {
    ...
    useEffect(() => {
        if(courseDetail?.ratingAndReview?.length > 0){
            const count = GetAvgRating(courseDetail?.ratingAndReview);
            console.log("RESULYT....",count)
            setAvgReviewCount(count);
            console.log("getCourseDetails -> count", parseInt(count));
            }
    }, [courseDetail?.ratingAndReview]);
                ...
                <div className='relative block max-h-[30rem] lg:hidden'>
                  ...
                  <img src={courseDetail?.thumbnail} alt="course img" />
                    ...
                    <div className='z-30 my-5 flex flex-col justify-center gap-4 py-5 text-lg text-richblack-5'>  
                        ...
                        {/* student enrolled */}
                        <span className='text-richblack-200'>{ courseDetail?.studentEnrolled?.length || 0 } students enrolled</span>
                    </div>
                    <div>
                    ...
                    <div className='flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden'>
                        ...
                        <p className='space-x-3 pb-4 text-3xl font-semibold text-richblack-5'>
                            ...
                            <>
                            {
                                alreadyEnrolled ? <button onClick={()=>{navigate("/dashboard/enrolled-courses")}}  className='bg-yellow-5'>Go to Course</button> : <button onClick={handelPayment} className='bg-yellow-5 Button'>Buy Now</button>
                            }
                            {
                                alreadyEnrolled ? (<div></div>) : 
                                (
                                    cart?.find((item) => item?._id === courseDetail?._id) ?
                                    (<button onClick={()=>{navigate("/dashboard/cart")}} className='blackButton bg-yellow-5 text-richblack-5'>Go to Cart</button>) :
                                    (<button onClick={handelAddToCart} className='blackButton bg-yellow-5 text-richblack-5'>Add to Cart</button>)
                                )
                            }
                            </>
                            }
                    ...
                    <div className='flex flex-col gap-4 rounded-md bg-richblack-700 p-4 text-richblack-5'>
                        ...
                        <div className='px-4'>
                            ...
                            <div className='flex flex-col gap-4'>
                                ...
                                <>
                                {
                                    alreadyEnrolled ? <button onClick={()=>{navigate("/dashboard/enrolled-courses")}} className='bg-yellow-5 text-black rounded-md px-3 py-2 yellowButton'>Go to Course</button> : <button onClick={handelPayment} className='bg-yellow-5 text-black rounded-md px-3 py-2 yellowButton'>Buy Now</button>
                                }
                                {
                                alreadyEnrolled ? (<div></div>) : 
                                (
                            ...
                            <div className=''>
                                ...
                                <div className='flex flex-col justify-center items-center gap-3 text-sm text-caribbeangreen-100'>
                                    {
                    ...
                    <div className='max-w-[830px] '>
                        <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex flex-wrap justify-between gap-2'>
                                ...
                                <span>{courseDetail?.courseSection?.length} Section(s)</span>
                                <span>{courseDetail?.courseSection?.reduce((acc, item) => acc + item?.subSection?.length, 0)} Lecture(s)</span>
                                </div>
                        ...
                        <div className='py-4'>
                            {
                                courseDetail?.courseSection?.map((item, index) => (
                    ...
                    <div className='my-8 border border-richblack-600 p-3 md:p-8'>
                        ...
                        <div className='mt-5'>
                            <div className='flex items-center gap-4'>
                                ...
                                {
                                    courseDetail?.ratingAndReview?.map((item, index) => (
